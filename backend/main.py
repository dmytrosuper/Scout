...
from apps.openai.main import get_openai_models_direct, OPENAI_API_KEY, OPENAI_API_URL
    ...
    async def dispatch(self, request: Request, call_next):
        if request.method == "POST" and (
            ...
            if not app.state.MODELS or len(app.state.MODELS) == 0:
                res = await get_openai_models_direct(OPENAI_API_URL, OPENAI_API_KEY)
                if res != None:
                    # model = res[model_id]
                    model = next((item for item in res if item['id'] == 'id2'), None)
                else :
                    model = {
                        "id": "gpt-4-turbo",
                        "name": "gpt-4-turbo",
                ...
                model = next((item for item in app.state.MODELS if item['id'] == model_id), None)
            ...
            if "pipeline" not in next((item for item in app.state.MODELS if item['id'] == model_id), None):
                if "chat_id" in data:
                    del data["chat_id"]
...
async def add_pipeline(form_data: AddPipelineForm, user=Depends(get_admin_user)):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {key}"}
        r = requests.post(
            f"{url}/pipelines/add", headers=headers, json={"url": form_data.url}, proxies=[proxies]
        )
...
async def get_pipelines(urlIdx: Optional[int] = None, user=Depends(get_admin_user)):
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {key}"}
        r = requests.get(f"{url}/pipelines", headers=headers, proxies=[proxies])
...
async def get_pipeline_valves(
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {key}"}
        r = requests.get(f"{url}/{pipeline_id}/valves", headers=headers, proxies=[proxies])
...
async def get_pipeline_valves_spec(
    ...
    try:
        ...
        headers = {"Authorization": f"Bearer {key}"}
        r = requests.get(f"{url}/{pipeline_id}/valves/spec", headers=headers, proxies=[proxies])
