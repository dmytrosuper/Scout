<script lang="ts">
	...
	import { chatCompleted, getModels } from '$lib/apis';
	import { Editor, rootCtx, defaultValueCtx, commandsCtx } from '@milkdown/core';
	import { listener, listenerCtx } from '@milkdown/plugin-listener';
	import { commonmark, toggleEmphasisCommand, toggleStrongCommand, toggleInlineCodeCommand, toggleLinkCommand, insertHrCommand, createCodeBlockCommand, insertImageCommand, wrapInHeadingCommand, wrapInOrderedListCommand, wrapInBlockquoteCommand, wrapInBulletListCommand } from '@milkdown/preset-commonmark';
  ...
  // pdfMake.vfs = pdfFonts.pdfMake.vfs;
	...
	const editorPanel = (dom: any) => {
		Editor.make()
			.config((ctx) => {
				ctx.set(rootCtx, dom);
				ctx.set(defaultValueCtx, $answers);
				ctx.get(listenerCtx).updated((ctx, doc, prevDoc) => {
					answers.set(getMarkdown()(ctx));
				});
			})
			.use(listener)
			.config(nord)
	...
	// 		editor.action((ctx) => {
	// 				const commandManager = ctx.get(commandsCtx);
	// 				commandManager.call(togglest.key);
	// 		});
	// };
	...
	const insertImage = (src, alt) => {
			editor.action((ctx) => {
					const commandManager = ctx.get(commandsCtx);
					commandManager.call(insertImageCommand.key, { src, alt });
			});
	};
	...
	// 		editor.action((ctx) => {
	// 				const commandManager = ctx.get(commandsCtx);
	// 				commandManager.call('InsertTable', { rows, cols });
	// 		});
	// };
	...
	const initNewChat = async () => {
		...
		if ($page.url.searchParams.get('models')) {
			...
			if (!$models.length) {
				(async () => {
					// const model = await getModels(localStorage.token);
					const model = await getOpenAIModelsDirect(OPENAI_API_URL, OPENAI_API_KEY);
					models.set(model);
				})();
			}
			selectedModels = ['gpt-4-turbo'];
	...
	const generateChatTitle = async (userPrompt) => {
		if ($settings?.title?.auto ?? true) {
			...
			const titleModelId =
				model?.name.startsWith('gpt-') ?? false
			...
			const title = await generateTitle(
				...
				userPrompt,
				$chatId,
				titleModel?.name.startsWith('gpt-') ?? false
					? `${OPENAI_API_BASE_URL}`
	...
	const generateChatSearchQuery = async (modelId: string, prompt: string) => {
		...
		return await generateSearchQuery(
			...
			previousMessages,
			prompt,
			taskModel?.name.startsWith('gpt-') ?? false
				? `${OPENAI_API_BASE_URL}`
				: `${OLLAMA_API_BASE_URL}/v1`
		);
	};
...
{#if !chatIdProp || (loaded && chatIdProp)}
	...
	>
		...
		<div class="flex flex-col flex-auto">
			{#if $docMode}
				<button
					class="flex w-fit py-2 px-3 bg-[#FE8157] hover:bg-[#d37150] transition text-white mr-10 ml-auto rounded-md"
					on:click={() => {
						const pdfmakeContent = htmlToPdfmake(document.querySelector('.milkdown > .ProseMirror')?.innerHTML);
						const docDefinition = {
							content: pdfmakeContent,
						};
						const title = chat.chat.title.split('\n')[0];
						pdfMake.createPdf(docDefinition).download(`${title || 'mydoc'}.pdf`);
					}}
				>
				...
				<div class="toolbarContainer">
					<div class="toolbar">
						<button on:click={toggleBold}>
							<i class="material-icons-outlined icon">format_bold</i>
						</button>
						<button on:click={toggleItalic}>
							<i class="material-icons-outlined icon">format_italic</i>
						</button>
						<!-- <button on:click={() => toggleHeading(1)}>
							<span class="icon">H1</span>
						</button>
						<button on:click={() => toggleHeading(2)}>
							<span class="icon">H2</span>
						</button>
						<button on:click={() => toggleHeading(3)}>
							<span class="icon">H3</span>
						</button> -->
						<button on:click={toggleBulletList}>
							<i class="material-icons-outlined icon">format_list_bulleted</i>
						</button>
						<button on:click={toggleOrderedList}>
							<i class="material-icons-outlined icon">format_list_numbered</i>
						</button>
						<button on:click={toggleBlockquote}>
							<i class="material-icons icon">format_quote</i>
						</button>
						<!-- <button on:click={toggleCodeBlock}>
							<i class="material-icons-outlined icon">code</i>
						</button> -->
						<button on:click={toggleCodeInline}>
							<i class="material-icons-outlined icon">code</i>
						</button>
						<button on:click={insertHorizontalRule}>
							<i class="icon">―</i>
						</button>
					</div>
				</div>
				<div use:editorPanel class="editor" />
			{:else}
				<div
