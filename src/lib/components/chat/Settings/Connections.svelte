<script lang="ts">
	...
	const verifyOpenAIHandler = async (idx) => {
		...
		models.set(await getOpenAIModelsDirect(OPENAI_API_URL, OPENAI_API_KEY));
	};
	...
	const updateOpenAIHandler = async () => {
		...
		OPENAI_API_KEYS = await updateOpenAIKeys(localStorage.token, OPENAI_API_KEYS);
		// await models.set(await getModels());
		await models.set(await getOpenAIModelsDirect(OPENAI_API_URL, OPENAI_API_KEY));
	};
...
>
	...
	<div class="flex justify-end pt-3 text-sm font-medium">
		<button
			class="  px-4 py-2 bg-[#FE8157] hover:bg-[#d47352] text-gray-100 transition rounded-lg"
			type="submit"
		>
			{$i18n.t('Save')}
		</button>
